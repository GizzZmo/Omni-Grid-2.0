...
const App: React.FC = () => {
  ...
  return (
    <div
      className={`h-screen w-screen flex flex-col bg-slate-950 text-slate-200 overflow-hidden relative selection:bg-fuchsia-500/30 ${settings.scanlines ? 'scanlines' : ''}`}
    >
      ...
      <div
        className="absolute top-0 left-0 w-full z-50 flex justify-between items-center p-4 pointer-events-none"
        role="banner"
      >
        {/* Brand */}
      ...
      {!isFrozen && (
        <div
          className="fixed bottom-6 left-1/2 transform -translate-x-1/2 z-50 max-w-[90vw] overflow-x-auto custom-scrollbar"
          role="navigation"
          aria-label="Dock"
        >
...
const DockItem = ({
  ...
  return (
    ...
    >
      ...
      {active && (
        <div
          className={`absolute -bottom-2 w-1 h-1 rounded-full ${color} shadow-[0_0_5px_currentColor]`}
        ></div>
      )}
    </button>
  );
