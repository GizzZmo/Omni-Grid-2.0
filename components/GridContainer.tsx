...
const WritePad = lazy(() =>
  import('../widgets/WritePad').then(m => ({ default: m.WritePad }))
);
...
const DocuHub = lazy(() =>
  import('../widgets/DocuHub').then(m => ({ default: m.DocuHub }))
);
const GitPulse = lazy(() =>
  import('../widgets/GitPulse').then(m => ({ default: m.GitPulse }))
);
...
const NewsFeed = lazy(() =>
  import('../widgets/NewsFeed').then(m => ({ default: m.NewsFeed }))
);
const CipherPad = lazy(() =>
  import('../widgets/CipherPad').then(m => ({ default: m.CipherPad }))
);
const PDFViewer = lazy(() =>
  import('../widgets/PDFViewer').then(m => ({ default: m.PDFViewer }))
);
...
const MacroNet = lazy(() =>
  import('../widgets/MacroNet').then(m => ({ default: m.MacroNet }))
);
const ChainPulse = lazy(() =>
  import('../widgets/ChainPulse').then(m => ({ default: m.ChainPulse }))
);
const RegRadar = lazy(() =>
  import('../widgets/RegRadar').then(m => ({ default: m.RegRadar }))
);
...
const PromptLab = lazy(() =>
  import('../widgets/PromptLab').then(m => ({ default: m.PromptLab }))
);
...
export const GridContainer: React.FC = () => {
  ...
  const widgetComponents = useMemo(
    () => ({
      ...
      SCRATCHPAD: (
        <WidgetShell id="SCRATCHPAD" title="Neural Scratchpad" icon={<BrainCircuit size={14} />} accentColor="text-fuchsia-400" className="h-full"
      ...
      FOCUS_HUD: (
        <WidgetShell id="FOCUS_HUD" title="Focus HUD" icon={<Activity size={14} />} accentColor="text-cyan-400" className="h-full"
      ...
      ASSET: (
        <WidgetShell id="ASSET" title="Asset Command" icon={<TrendingUp size={14} />} accentColor="text-emerald-400" className="h-full"
      ...
      WRITEPAD: (
        <WidgetShell id="WRITEPAD" title="WritePad" icon={<PenTool size={14} />} accentColor="text-rose-400" className="h-full"
      ...
      WEATHER: (
        <WidgetShell id="WEATHER" title="Weather" icon={<Cloud size={14} />} accentColor="text-sky-400" className="h-full"
  ...
  return (
    ...
    >
      {activeLayout.map(item => (
        <div key={item.i} data-grid={item} className={`relative ${item.i === 'GHOST' ? 'z-0' : 'z-10'}`}>
