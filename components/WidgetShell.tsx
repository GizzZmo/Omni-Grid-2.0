...
export const WidgetShell = React.forwardRef<HTMLDivElement, WidgetShellProps>(
  ({ id, title, children, icon, accentColor, style, className, onMouseDown, onMouseUp, onTouchEnd, onExport, onImport, ...props }, ref) => {
    ...
    const borderColor = accentColor.replace('text-', 'border-').replace('400', '500').replace('300', '500') || 'border-cyan-500';
    ...
    return (
      ...
      >
        ...
        <div className={`absolute inset-0 pointer-events-none border ${borderColor} opacity-30 group-hover:opacity-60 transition-opacity`} style={{ clipPath: 'polygon(10px 0, 100% 0, 100% calc(100% - 10px), calc(100% - 10px) 100%, 0 100%, 0 10px)' }}></div>
        
        {/* Glowing Corner Accents */}
        <div className={`absolute top-0 left-0 w-2 h-2 border-t-2 border-l-2 ${borderColor} opacity-80`}></div>
        <div className={`absolute bottom-0 right-0 w-2 h-2 border-b-2 border-r-2 ${borderColor} opacity-80`}></div>

        {/* Header */}
        <div className={`drag-handle relative flex items-center justify-between px-3 py-2 border-b border-white/5 bg-gradient-to-r from-white/5 to-transparent ${isLayoutLocked ? 'cursor-default' : 'cursor-grab active:cursor-grabbing'}`}>
            ...
            <div className="relative flex items-center gap-2 z-10">
                <span className={`${accentColor} drop-shadow-[0_0_5px_rgba(255,255,255,0.3)]`}>{icon}</span>
                <span className="font-display text-xs font-bold tracking-widest text-slate-200 uppercase">{title}</span>
            </div>
