...
export const WidgetArchitect: React.FC = () => {
  ...
  const [logicNodes, setLogicNodes] = useState<Array<{id: string, label: string, type: 'DATA'|'LOGIC'|'UI'}>>([]);
        ...
        <div className="flex-1 flex flex-col gap-4">
           <div className="bg-slate-900/50 p-4 rounded border border-indigo-500/20">
             <div className="flex justify-between items-center mb-2">
                <label className="text-xs text-indigo-400 font-bold uppercase">Prototype Creator</label>
           ...
           <div className="mt-auto text-[10px] text-slate-500 p-2 border-l-2 border-slate-800">
              <strong className="text-slate-400">Architect Mode:</strong> Use this tool to rapidly prototype new widgets. The code generated in the next tab can be used to build real extensions.
           </div>
        </div>
      )}
      ...
      {activeTab === 'CODE' && (
        <div className="flex-1 relative group bg-slate-950 border border-slate-800 rounded overflow-hidden">
          <div className="absolute top-0 left-0 w-full bg-slate-900 p-1 border-b border-slate-800 flex justify-between items-center px-2">
            <span className="text-[10px] text-slate-400 flex items-center gap-1"><Code size={10}/> Component.tsx</span>
            <button 
            ...
            {logicNodes.length > 0 ? (
               <div className="flex flex-col gap-4 items-center w-full">
                  {logicNodes.map((node, i) => (
                     <div key={node.id} className="flex flex-col items-center w-full animate-in fade-in slide-in-from-bottom-2" style={{ animationDelay: `${i * 100}ms` }}>
                        <div className={`
                        ...
                        `}>
                           ...
                           <div className="flex-1">
                              <div className="text-[8px] opacity-50 font-bold tracking-widest">{node.type} LAYER</div>
