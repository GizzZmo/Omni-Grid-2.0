...
import { FileJson, ArrowRightLeft, FileType, Check, Copy, GripHorizontal, Upload } from 'lucide-react';
...
export const UniversalTransformer: React.FC = () => {
  ...
  const calculateConversion = (val: number, from: string) => {
    if (isNaN(val)) return '...';
    switch (from) {
      case 'px': return `${(val / 16).toFixed(3).replace(/\.000$/, '')}rem`;
      case 'rem': return `${(val * 16).toFixed(0)}px`;
      case 'epoch': return new Date(val * 1000).toLocaleString();
      ...
      case 'kg': return `${(val * 2.20462).toFixed(2)}lb`;
  ...
  return (
    <div className="h-full flex flex-col gap-4">
      ...
      <div className="flex gap-2 p-1 bg-slate-900 rounded-lg">
        <button
          onClick={() => { setActiveTab('json-csv'); setCopied(false); }}
        ...
        <button
          onClick={() => { setActiveTab('unit'); setCopied(false); }}
      ...
      {activeTab === 'json-csv' ? (
        <div className="flex-1 flex flex-col gap-2 min-h-0">
          ...
          <div className="flex gap-2 justify-center">
            ...
            <button onClick={convertToCSV} className="bg-slate-800 hover:bg-slate-700 px-3 py-1 rounded text-xs text-indigo-400 font-medium">to CSV</button>
            <button onClick={convertToJSON} className="bg-slate-800 hover:bg-slate-700 px-3 py-1 rounded text-xs text-indigo-400 font-medium">to JSON</button>
          </div>
        ...
        <div className="flex flex-col gap-4 p-2">
            ...
            <div className="bg-slate-900 p-4 rounded-lg border border-slate-800 text-center relative overflow-hidden group">
                ...
                <label className="text-[10px] uppercase text-slate-500 font-bold block mb-1">Result</label>
