        ...
        <div className="flex-1 flex flex-col gap-4 overflow-y-auto custom-scrollbar">
          <div className="grid grid-cols-2 gap-2">
            ...
            <div className="bg-slate-900/50 p-2 border border-slate-800 flex flex-col gap-1">
              ...
              <span className={`text-xs ${vaultStatusColor} animate-pulse`}>{vaultStatusLabel}</span>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar">
            {visibleWidgets.map(widgetId => {
              ...
              return (
                <div
                  key={widgetId}
                  className="flex items-center justify-between px-2 py-1.5 border-b border-slate-800/50 hover:bg-slate-800/30 group"
                >
                  <span className="w-10 text-[10px] text-emerald-600 font-mono">{stats.pid}</span>
                  <span className="flex-1 text-xs text-slate-300 font-bold truncate pr-2">
                    {widgetId}
                  </span>
      ...
      {activeTab === 'LOGS' && (
        <div className="flex-1 bg-black/40 border border-slate-800 p-2 overflow-y-auto custom-scrollbar font-mono text-[10px] leading-relaxed">
          {logs.map((log, i) => (
            <div
              key={i}
              className="mb-1 border-l-2 border-slate-800 pl-2 hover:border-cyan-500/50 hover:bg-white/5 transition-colors"
            >
      ...
      {activeTab === 'SETTINGS' && (
        <div className="flex-1 flex flex-col gap-3 p-1 overflow-y-auto custom-scrollbar">
          <div className="flex items-center justify-between p-2 bg-slate-900/50 border border-slate-800 rounded">
            ...
            >
              <div
                className={`absolute top-0.5 left-0.5 w-3 h-3 bg-white rounded-full transition-transform ${settings.scanlines ? 'translate-x-4' : 'translate-x-0'}`}
              ></div>
            </button>
          </div>
          ...
          <div className="flex flex-col gap-2 p-2 bg-slate-900/50 border border-violet-900/40 rounded">
            <div className="flex items-center justify-between">
              ...
              <span className={`text-[10px] font-bold ${vaultStatusColor}`}>{vaultStatusLabel}</span>
            </div>
