...
import { Search, Globe, ShieldCheck, Loader2, ExternalLink, Link as LinkIcon, AlertCircle } from 'lucide-react';
...
export const ResearchBrowser: React.FC = () => {
  ...
  return (
    <div className="h-full flex flex-col gap-2 font-mono">
        <form onSubmit={handleSearch} className="flex gap-2 bg-slate-900 p-2 rounded border border-slate-800 shrink-0">
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar p-4 animate-in fade-in slide-in-from-bottom-2 duration-500">
                    ...
                    {content.chunks.length > 0 && (
                        <div className="border-t border-slate-800 pt-2 mt-2">
                            <div className="text-[10px] font-bold text-slate-500 uppercase mb-2">Source Telemetry</div>
                            <div className="flex flex-col gap-1">
                                {content.chunks.map((chunk, i) => {
                                    if (chunk.web) {
                                        return (
                                            <a key={i} href={chunk.web.uri} target="_blank" rel="noreferrer" className="flex items-center gap-2 p-1.5 bg-slate-900/50 rounded hover:bg-slate-800 border border-slate-800 hover:border-cyan-500/30 transition-all group">
                                                <LinkIcon size={10} className="text-slate-500 group-hover:text-cyan-400" />
                                                <div className="flex flex-col overflow-hidden">
                                                    <span className="text-[10px] text-cyan-400 font-bold truncate">{chunk.web.title}</span>
                                                    <span className="text-[8px] text-slate-600 truncate">{chunk.web.uri}</span>
                                                </div>
                                                <ExternalLink size={10} className="ml-auto text-slate-600 group-hover:text-white" />
                                            </a>
                                        );
                                    }
                                    return null;
