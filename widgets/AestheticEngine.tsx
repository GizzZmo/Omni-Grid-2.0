    ...
    try {
      ...
      const newTheme: AppTheme = {
          ...
          colors: themeData.colors || {
              background: '#000000', surface: '#111111', primary: '#ffffff', secondary: '#888888', text: '#eeeeee', accent: '#00ff00'
          },
  ...
  return (
    <div className="h-full flex flex-col gap-3">
        ...
        <div className="flex bg-slate-900 p-1 rounded-lg">
             <button onClick={() => setActiveMode('TEXT')} className={`flex-1 py-1 text-xs font-bold rounded transition-colors ${activeMode === 'TEXT' ? 'bg-pink-600 text-white' : 'text-slate-500'}`}>Text Prompt</button>
             <button onClick={() => setActiveMode('IMAGE')} className={`flex-1 py-1 text-xs font-bold rounded transition-colors ${activeMode === 'IMAGE' ? 'bg-pink-600 text-white' : 'text-slate-500'}`}>Mood Board</button>
        </div>
        ...
        <div className="flex-1 flex flex-col gap-4">
             ...
             <div className="bg-slate-900/50 border border-slate-800 rounded p-3">
                 <div className="text-[10px] uppercase text-slate-500 font-bold mb-2">Current System State</div>
                 <div className="grid grid-cols-2 gap-2 text-xs">
                     <div className="flex items-center gap-2">
                         <div className="w-4 h-4 rounded-full border border-white/10" style={{backgroundColor: theme.colors.background}}></div>
                         <span className="text-slate-400">BG</span>
                     </div>
                     <div className="flex items-center gap-2">
                         <div className="w-4 h-4 rounded-full border border-white/10" style={{backgroundColor: theme.colors.primary}}></div>
                         <span className="text-slate-400">Pri</span>
                     </div>
                     <div className="flex items-center gap-2">
                         <div className="w-4 h-4 rounded-full border border-white/10" style={{backgroundColor: theme.colors.secondary}}></div>
                         <span className="text-slate-400">Sec</span>
                     </div>
                     <div className="flex items-center gap-2">
                         <div className="w-4 h-4 rounded-full border border-white/10" style={{backgroundColor: theme.colors.accent}}></div>
