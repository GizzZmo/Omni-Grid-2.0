...
import { TrendingUp, Plus, Brain, Trash2, Loader2, Coins, Play, Zap, AlertTriangle, Lightbulb } from 'lucide-react';
...
export const AssetCommand: React.FC = () => {
  ...
  const analyzeSmartGrid = async () => {
    ...
    try {
      ...
      const context = tickers.map(t => `${t}: ${ASSET_CONTEXT[t] || 'Speculative Asset'}`).join('\n');
      ...
      {activeTab === 'TICKERS' && (
          <div className="flex-1 flex flex-col gap-2 min-h-0">
            <form onSubmit={addTicker} className="flex gap-2">
                ...
                />
                <button type="submit" className="bg-slate-800 p-1 rounded hover:bg-slate-700 text-emerald-400">
                <Plus size={14} />
                </button>
            </form>
            ...
            <div className="flex-1 flex flex-col gap-1 overflow-y-auto custom-scrollbar">
                {tickers.map(t => (
                    <div key={t} className="flex items-center justify-between p-2 bg-slate-900/50 rounded border border-white/5 hover:border-emerald-500/30 group">
                        <div className="flex flex-col">
                            <div className="flex items-center gap-2">
                                <span className="p-1 bg-slate-800 rounded text-slate-400">
                                    {['BTC','ETH','SOL','USDT','XRP'].includes(t) ? <Coins size={12}/> : <TrendingUp size={12}/>}
                                </span>
                                <span className="font-bold text-slate-200 text-sm">{t}</span>
                            </div>
                            {ASSET_CONTEXT[t] && (
                                <span className="text-[9px] text-slate-500 ml-7">{ASSET_CONTEXT[t].split(':')[0]}</span>
                            )}
                        </div>
                        <button onClick={() => removeTicker(t)} className="opacity-0 group-hover:opacity-100 text-red-400 hover:text-red-300">
                            <Trash2 size={12} />
                        </button>
                    </div>
                ))}
          ...
          <div className="flex-1 flex flex-col gap-2 min-h-0">
              <div className="bg-slate-900/50 border border-slate-800 p-2 rounded flex flex-col gap-2">
                  ...
                  <div className="flex gap-1">
                      <select value={logicAsset} onChange={e => setLogicAsset(e.target.value)} className="w-16 bg-slate-800 text-[10px] rounded border border-slate-700 text-white outline-none">
                          {tickers.map(t => <option key={t} value={t}>{t}</option>)}
                      </select>
                      <select value={logicOp} onChange={e => setLogicOp(e.target.value)} className="w-10 bg-slate-800 text-[10px] rounded border border-slate-700 text-white outline-none">
                          <option>{'<'}</option>
                          <option>{'>'}</option>
                          <option>=</option>
                      </select>
                  ...
                  <div className="flex gap-1 items-center">
                      ...
                      <select value={logicAction} onChange={e => setLogicAction(e.target.value)} className="flex-1 bg-slate-800 text-[10px] rounded border border-slate-700 text-white outline-none">
                      ...
                      <button onClick={addAutomation} className="bg-emerald-700 hover:bg-emerald-600 text-white p-1 rounded">
                          <Plus size={12} />
                      </button>
                  </div>
              </div>
              ...
              <div className="flex-1 overflow-y-auto custom-scrollbar flex flex-col gap-1">
                  {automations.map((rule, i) => (
                      <div key={i} className="bg-emerald-900/20 border border-emerald-900/50 p-2 rounded text-[10px] font-mono text-emerald-300 flex justify-between items-center">
                          <span>{rule}</span>
                          <button onClick={() => setAutomations(automations.filter((_, idx) => idx !== i))} className="text-emerald-700 hover:text-red-400"><Trash2 size={10}/></button>
                      </div>
                  ))}
