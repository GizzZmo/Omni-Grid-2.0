...
export const HelpDesk: React.FC = () => {
  ...
  const GUIDES_DATA = [
      // Core System
      {
          id: 'g_sys',
          title: 'System Core',
          steps: ['Monitor simulated CPU/Mem usage.', 'Kill unresponsive widgets via the Task Manager tab.', 'Toggle CRT Scanlines/Audio.', 'Perform Factory Reset.']
      },
      {
          id: 'g_help',
          title: 'Help Desk',
          steps: ['Use the Search bar to find definitions.', 'Read Q&A for troubleshooting.', 'Browse these guides for operational protocols.']
      },
      // Tools
      {
          id: 'g_trans',
          title: 'Transformer',
          steps: ['Paste JSON or CSV data.', 'Click convert buttons.', 'Drag output to other widgets.', 'Use the "Units" tab for PX/REM conversion.']
      },
      {
          id: 'g_scratch',
          title: 'Neural Notes',
          steps: ['Type or paste text.', 'Highlight text to reveal AI Context Menu.', 'Use Cmd+K shortcut.', 'Drag files into the text area to import.']
      },
      {
          id: 'g_focus',
          title: 'Focus HUD',
          steps: ['Add tasks to the Kanban queue.', 'Click Play to start 25m Pomodoro.', 'Toggle Brown Noise for immersion.', 'Mark tasks complete.']
      },
      {
          id: 'g_dev',
          title: 'Dev Optic',
          steps: ['Paste JWT strings to decode payload.', 'Test Regex patterns against strings.', 'View real-time matches and flags.']
      },
      {
          id: 'g_cipher',
          title: 'Cipher Vault',
          steps: ['Generate UUID v4 or NanoIDs.', 'Hash text using SHA-256/512.', 'Base64 encode/decode strings.']
      },
      {
          id: 'g_chroma',
          title: 'Chroma Lab',
          steps: ['Pick a base color.', 'View generated shades (50-900).', 'Click any shade to copy the Tailwind class.', 'Use the pipette visualizer.']
      },
      {
          id: 'g_temp',
          title: 'Temporal Nexus',
          steps: ['View current UTC/Local time.', 'Switch to "On This Day".', 'Use AI to fetch historical facts for today\'s date.']
      },
      {
          id: 'g_sonic',
          title: 'Sonic Arch',
          steps: ['Reference the Circle of Fifths.', 'View compatible Major/Minor chords.', 'Use the Metronome for timing.', 'Play reference tones.']
      },
      {
          id: 'g_calc',
          title: 'Quantum Calc',
          steps: ['Perform standard arithmetic.', 'Use scientific functions (sin, cos, log).', 'View history of the last result.', 'Handle complex expressions.']
      },
      // Financial
      {
          id: 'g_asset',
          title: 'Asset Command',
          steps: ['Add tickers (e.g. BTC, ETH).', 'Define logic rules (IF BTC < 50k THEN ALERT).', 'Generate AI Portfolio Analysis.']
      },
      {
          id: 'g_poly',
          title: 'Polyglot Box',
          steps: ['Select Source and Target languages.', 'Paste code snippet.', 'Click Translate to transpile logic via AI.', 'Copy result.']
      },
      {
          id: 'g_write',
          title: 'WritePad',
          steps: ['Select a template (Email, Invoice).', 'Use AI Draft to generate content from prompts.', 'Drag & Drop Cross-Talk content to append.']
      },
      {
          id: 'g_weather',
          title: 'Weather Station',
          steps: ['Enter City or Zip Code.', 'View current conditions.', 'Monitor Humidity and Wind speeds.']
      },
      {
          id: 'g_valuta',
          title: 'Valuta Exchange',
          steps: ['Select input currency (USD).', 'Select target currency (EUR).', 'Enter amount to see real-time conversion rates.']
      },
      // Advanced
      {
          id: 'g_arch',
          title: 'Widget Architect',
          steps: ['Describe a widget idea (e.g. "Crypto Tracker").', 'Click Generate Prototype.', 'View Logic Graph and Copy React Code.']
      },
      {
          id: 'g_theme',
          title: 'Aesthetic Engine',
          steps: ['Describe a vibe (e.g. "Cyberpunk Red").', 'Upload an image to extract colors.', 'Apply generated theme to the entire app.']
      },
      {
          id: 'g_radio',
          title: 'Signal Radio',
          steps: ['Tune into SoundCloud sets.', 'Activate Microphone for visualizer.', 'Watch frequency bars react to audio.']
      },
      {
          id: 'g_sudoku',
          title: 'Sudoku Grid',
          steps: ['Select Difficulty.', 'Fill cells (1-9).', 'Use "Note Mode" to mark possibilities.', 'Clear board when finished.']
      },
      // Dev/Research
      {
          id: 'g_docu',
          title: 'DocuHub',
          steps: ['Browse offline documentation via DevDocs wrapper.', 'Access standard library refs.', 'Use external links for source.']
      },
      {
          id: 'g_git',
          title: 'Git Pulse',
          steps: ['Enter Personal Access Token (PAT).', 'Monitor open PRs.', 'Check build status and merge readiness.']
      },
      {
          id: 'g_track',
          title: 'Project Tracker',
          steps: ['Create tasks in "Todo".', 'Drag tasks between "Progress" and "Done".', 'Manage simple project kanban flow.']
      },
      {
          id: 'g_term',
          title: 'Web Terminal',
          steps: ['Execute JavaScript commands.', 'Access browser console features.', 'Test logic snippets locally.']
      },
      {
          id: 'g_news',
          title: 'News Feed',
          steps: ['Read latest headlines.', 'Refresh for updates.', 'Click external links to read full articles.']
      },
      {
          id: 'g_pad',
          title: 'Cipher Pad',
          steps: ['Enter a session key.', 'Unlock pad.', 'Type sensitive notes.', 'Lock to encrypt in memory.']
      },
      {
          id: 'g_pdf',
          title: 'PDF Viewer',
          steps: ['Drag and Drop PDF files.', 'Read whitepapers or docs inline.', 'Close file to reset.']
      },
      {
          id: 'g_browser',
          title: 'Research Browser',
          steps: ['Enter query (e.g. "React 19 features").', 'AI browses the web.', 'Read synthesized summary and click source citations.']
      },
      {
          id: 'g_cal',
          title: 'Secure Calendar',
          steps: ['Navigate months.', 'Click dates to add events.', 'Events are stored locally and encrypted.']
      },
      {
          id: 'g_macro',
          title: 'Macro Net',
          steps: ['Analyze Global Liquidity heatmap.', 'Check asset correlations against M2 supply.', 'Monitor inflation impact.']
      },
      {
          id: 'g_chain',
          title: 'Chain Pulse',
      ...
      },
      {
          id: 'g_reg',
          title: 'Reg Radar',
          steps: ['Monitor regulatory threat levels.', 'Read updates on SEC/MiCA actions.', 'Check jurisdiction status.']
      },
      {
          id: 'g_mkt',
          title: 'Market Feed',
          steps: ['View 24h prices for major assets.', 'Track percentage changes.', 'Auto-refreshes every 60s.']
      }
  ];
  ...
  return (
    <div className="h-full flex flex-col gap-3 font-mono">
        {/* Tab Header */}
        <div className="flex bg-slate-900/50 p-1 border border-slate-800 rounded-lg shrink-0" role="tablist">
            <button 
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar bg-slate-950/30 border border-slate-800/50 rounded-lg p-2 min-h-0">
            ...
            {activeTab === 'WIKI' && (
                <div className="flex flex-col gap-2">
                    {WIKI_DATA.filter(i => i.term.toLowerCase().includes(debouncedSearch.toLowerCase())).map(item => (
                        <div key={item.id} className="bg-slate-900/50 border border-slate-800 p-2 rounded hover:border-emerald-500/30 transition-colors group">
                            ...
                            <div className="text-[10px] text-slate-400 leading-relaxed pl-4 border-l border-slate-800">{item.def}</div>
                        </div>
                    ))}
                </div>
            )}
            ...
            {activeTab === 'QA' && (
                <div className="flex flex-col gap-2">
                    {QA_DATA.filter(i => i.q.toLowerCase().includes(debouncedSearch.toLowerCase())).map(item => (
                        <div key={item.id} className="bg-slate-900/50 border border-slate-800 rounded overflow-hidden">
                            <button 
                            ...
                            >
                                ...
                                <ChevronRight size={12} className={`text-slate-500 transition-transform ${expandedItem === item.id ? 'rotate-90' : ''}`} />
                            </button>
            ...
            {activeTab === 'GUIDES' && (
                 <div className="flex flex-col gap-3">
                    {GUIDES_DATA.filter(i => i.title.toLowerCase().includes(debouncedSearch.toLowerCase())).map(guide => (
                        <div key={guide.id} className="bg-slate-900/50 border border-slate-800 p-3 rounded hover:border-cyan-500/30 transition-all">
                            ...
                            <ul className="flex flex-col gap-1.5">
                                {guide.steps.map((step, idx) => (
                                    <li key={idx} className="text-[10px] text-slate-400 flex gap-2">
                                        <span className="text-slate-600 font-mono">{(idx + 1).toString().padStart(2, '0')}</span>
                                        {step}
                                    </li>
                                ))}
                            </ul>
