...
const BLANK_BOARD: Board = Array(9).fill(null).map(() => Array(9).fill(null));
...
export const SudokuGrid: React.FC = () => {
  ...
  const handleInput = useCallback((num: number) => {
    ...
    if (isNoteMode) {
      ...
      if (num === solution[r][c]) {
        ...
        // Check Win
        let isFull = true;
        for(let i=0; i<9; i++) for(let j=0; j<9; j++) if(newBoard[i][j] === null) isFull = false;
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === 'ArrowUp') setSelected(prev => prev ? [Math.max(0, prev[0]-1), prev[1]] : [0,0]);
      if (e.key === 'ArrowDown') setSelected(prev => prev ? [Math.min(8, prev[0]+1), prev[1]] : [0,0]);
      if (e.key === 'ArrowLeft') setSelected(prev => prev ? [prev[0], Math.max(0, prev[1]-1)] : [0,0]);
      if (e.key === 'ArrowRight') setSelected(prev => prev ? [prev[0], Math.min(8, prev[1]+1)] : [0,0]);
    };
    ...
    <div className="h-full flex flex-col gap-3 select-none">
      ...
      <div className="flex justify-between items-center bg-slate-900 p-2 rounded-lg border border-slate-800">
        <div className="flex gap-2">
            ...
            <button onClick={() => newGame()} className="p-1 hover:text-cyan-400 text-slate-500 transition-colors" title="Restart">
                <RotateCcw size={14} />
            </button>
        </div>
        
        <div className="flex gap-4 font-mono text-xs font-bold text-slate-300">
            <div className={`flex items-center gap-1 ${mistakes >= 3 ? 'text-red-500 animate-pulse' : ''}`}>
                <AlertCircle size={12}/> {mistakes}/3
            </div>
            <div className="flex items-center gap-1 w-16 justify-end">
                 {formatTime(timer)}
            </div>
            <button onClick={() => setIsPaused(!isPaused)} className="hover:text-cyan-400 transition-colors">
      ...
      <div className="flex-1 flex flex-col items-center justify-center relative">
        {gameWon && (
            <div className="absolute inset-0 z-20 bg-slate-950/90 flex flex-col items-center justify-center animate-in zoom-in">
                <Trophy size={48} className="text-yellow-400 mb-4 drop-shadow-[0_0_15px_rgba(250,204,21,0.5)]" />
                <div className="text-2xl font-bold text-white mb-2">SYSTEM CLEARED</div>
                <div className="text-sm text-slate-400 mb-4">Difficulty: {difficulty} | Time: {formatTime(timer)}</div>
                <button 
                    onClick={() => newGame()}
        ...
        {isPaused && !gameWon && (
            <div className="absolute inset-0 z-20 bg-slate-950/95 flex flex-col items-center justify-center">
                 ...
                 <button onClick={() => setIsPaused(false)} className="mt-4 text-cyan-400 hover:text-cyan-300 underline">Resume</button>
            </div>
        )}

        <div className="grid grid-cols-9 border-2 border-slate-600 bg-slate-800 shadow-2xl" style={{ aspectRatio: '1/1', maxHeight: '100%', maxWidth: '100%' }}>
          {board.map((row, r) => 
            row.map((cell, c) => {
              ...
              const borderR = (c + 1) % 3 === 0 && c !== 8 ? 'border-r-2 border-r-slate-500' : 'border-r border-r-slate-700';
              const borderB = (r + 1) % 3 === 0 && r !== 8 ? 'border-b-2 border-b-slate-500' : 'border-b border-b-slate-700';
                  ...
                  {cell !== null ? cell : (
                     ...
                     <div className="grid grid-cols-3 gap-[1px] w-full h-full p-0.5 pointer-events-none">
                         {[1,2,3,4,5,6,7,8,9].map(n => (
                             <div key={n} className="flex items-center justify-center text-[6px] leading-none text-fuchsia-400/80">
                                 {notes[r][c].has(n) ? n : ''}
                             </div>
                         ))}
                     </div>
