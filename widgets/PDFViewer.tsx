        ...
        {!fileUrl ? (
            ...
            <div className="flex-1 flex flex-col h-full">
                <div className="flex justify-end mb-1">
                    <button onClick={() => setFileUrl(null)} className="text-slate-500 hover:text-red-400 flex items-center gap-1 text-[10px]">
                        <X size={12}/> Close PDF
                    </button>
                </div>
                <iframe src={fileUrl} className="flex-1 w-full rounded border border-slate-800 bg-white" title="PDF Reader" />
            </div>
        )}
    </div>
  );
