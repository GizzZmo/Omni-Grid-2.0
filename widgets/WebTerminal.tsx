  ...
  return (
    <div className="h-full flex flex-col bg-black border border-slate-800 font-mono text-xs rounded-lg overflow-hidden">
        <div className="flex-1 overflow-y-auto p-2 custom-scrollbar flex flex-col gap-1">
            {history.map((entry, i) => (
                <div key={i} className={`whitespace-pre-wrap break-all ${entry.type === 'in' ? 'text-cyan-400' : entry.type === 'err' ? 'text-red-400' : 'text-slate-300'}`}>
                    {entry.type === 'in' ? '> ' : ''}{entry.content}
                </div>
            ))}
        ...
        <div className="p-2 bg-slate-900 border-t border-slate-800 flex gap-2 items-center">
            ...
            />
            <button onClick={() => setHistory([])} className="text-slate-600 hover:text-red-400"><Trash2 size={12}/></button>
            <button onClick={execute} className="text-slate-600 hover:text-cyan-400"><Play size={12}/></button>
        </div>
    </div>
  );
};
