...
export const ProjectTracker: React.FC = () => {
  const [lanes, setLanes] = useState({
      'todo': [{id: 1, text: 'Design DB Schema'}, {id: 2, text: 'Auth API'}],
    ...
    <div className="h-full flex flex-col gap-2">
        <div className="flex-1 flex gap-2 overflow-x-auto min-w-0">
            {(Object.keys(lanes) as Array<keyof typeof lanes>).map(lane => (
                <div key={lane} className="flex-1 min-w-[140px] bg-slate-900/50 rounded-lg border border-slate-800 flex flex-col">
                    <div className="p-2 border-b border-slate-800 flex justify-between items-center">
                        ...
                        <span className="text-[10px] bg-slate-800 px-1.5 rounded text-slate-500">{lanes[lane].length}</span>
                    </div>
                    <div className="flex-1 p-2 flex flex-col gap-2 overflow-y-auto custom-scrollbar">
                        {lanes[lane].map(task => (
                            <div 
                                ...
                                draggable
                                onDragStart={(e) => e.dataTransfer.setData('task', JSON.stringify({id: task.id, from: lane}))}
                            >
                                {task.text}
                            </div>
                        ))}
