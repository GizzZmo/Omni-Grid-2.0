...
const FlowNode = ({ label, subLabel, color, height, x, y }: { label: string, subLabel: string, color: string, height: number, x: string, y: string }) => (
  <div 
  ...
  >
      ...
      <div className="absolute left-8 top-1/2 -translate-y-1/2 flex flex-col w-24 pointer-events-none">
          <span className="text-[10px] font-bold text-slate-200 uppercase tracking-widest drop-shadow-md">{label}</span>
        ...
        {activeTab === 'HEATMAP' ? (
            <>
                ...
                <div className="flex-1 bg-slate-900/30 border border-slate-800 rounded p-2 relative flex flex-col justify-end min-h-0">
                    ...
                    <div className="flex items-end justify-between h-full gap-1 px-2 pb-1">
                        {[40, 45, 42, 55, 60, 58, 70, 85, 90, 88].map((h, i) => (
                            <div key={i} className="flex-1 flex flex-col justify-end gap-1 group relative h-full">
                                ...
                                <div 
                                    ...
                                    style={{ height: `${h + (Math.random() * 20 - 10)}%` }}
                                ></div>
                                {/* BTC Bar */}
                                <div 
                    ...
                    <div className="flex justify-between text-[8px] text-slate-600 mt-1 uppercase font-bold px-2">
                        <span>Q1-23</span><span>Q2-23</span><span>Q3-23</span><span>Q4-23</span><span>Q1-24</span>
                    </div>
                </div>
                ...
                <div className="h-auto shrink-0 flex flex-col gap-1">
                    ...
                    {correlationData.map((d) => (
                        <div key={d.asset} className="grid grid-cols-4 gap-1 items-center bg-slate-900/50 p-1.5 rounded border border-white/5 hover:border-blue-500/30 transition-colors">
                            <span className="text-xs font-bold text-slate-200 pl-1">{d.asset}</span>
                            <span className={`text-xs font-mono text-center ${d.m2 > 0.7 ? 'text-emerald-400' : 'text-slate-400'}`}>
                                {d.m2 > 0 ? '+' : ''}{d.m2}
                            </span>
                            <span className="text-xs font-mono text-slate-400 text-center">{d.inflation}</span>
                            <span className={`text-xs font-mono text-center ${d.yield > 0 ? 'text-blue-400' : 'text-red-400'}`}>
                                {d.yield}%
                            </span>
                        </div>
                    ))}
            ...
            <div className="flex-1 bg-slate-950/50 border border-slate-800 rounded relative overflow-hidden flex items-center justify-center">
                ...
                <svg className="absolute inset-0 w-full h-full pointer-events-none z-0">
                    {/* FIAT to STABLES */}
                    <FlowLink startX={40} startY={80} endX={140} endY={80} thickness={20} color="#10b981" delay={0} />
                    
                    {/* STABLES to ASSETS */}
                    <FlowLink startX={165} startY={80} endX={260} endY={40} thickness={12} color="#3b82f6" delay={0.5} /> {/* BTC */}
                    <FlowLink startX={165} startY={80} endX={260} endY={100} thickness={8} color="#8b5cf6" delay={0.8} />  {/* ETH */}
                    <FlowLink startX={165} startY={80} endX={260} endY={150} thickness={4} color="#f59e0b" delay={1.1} />  {/* SOL */}

                    {/* ETH to L2s */}
                    <FlowLink startX={285} startY={100} endX={350} endY={100} thickness={4} color="#6366f1" delay={1.5} />
                </svg>

                {/* Nodes Layer */}
                <div className="w-full h-full relative z-10">
                    {/* Column 1: Source */}
                    <FlowNode x="5%" y="30%" height={80} color="bg-emerald-500" label="FIAT" subLabel="Global M2" />

                    {/* Column 2: Liquidity */}
                    <FlowNode x="35%" y="30%" height={80} color="bg-blue-500" label="STABLES" subLabel="$150B Liquidity" />

                    {/* Column 3: Assets */}
                    <FlowNode x="65%" y="10%" height={60} color="bg-orange-500" label="BTC" subLabel="Store of Value" />
                    <FlowNode x="65%" y="40%" height={50} color="bg-fuchsia-500" label="ETH" subLabel="Settlement" />
                    <FlowNode x="65%" y="70%" height={40} color="bg-cyan-500" label="SOL" subLabel="Execution" />
                    
                    {/* Column 4: L2/Apps */}
                    <FlowNode x="90%" y="45%" height={30} color="bg-indigo-500" label="L2s" subLabel="Arbitrum/Base" />
                </div>
