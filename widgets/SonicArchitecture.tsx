      ...
      {activeTab === 'THEORY' ? (
          <div className="flex-1 flex flex-col items-center justify-center gap-4">
              ...
              <div className="w-full bg-slate-900/50 p-4 rounded-lg border border-slate-800">
                  <div className="text-[10px] uppercase text-slate-500 font-bold mb-2 text-center">Compatible Chords (Key of {rootNote})</div>
                  <div className="flex flex-col gap-2">
                      <div className="flex gap-2 justify-center">
                          {chords.major?.map((c, i) => (
                              <div key={c} className={`w-12 h-12 flex items-center justify-center rounded font-bold text-slate-900 ${i === 1 ? 'bg-fuchsia-400 scale-110 shadow-lg shadow-fuchsia-900/50' : 'bg-fuchsia-200/70'}`}>
                                  {c}
                              </div>
                          ))}
                      </div>
                      <div className="flex gap-2 justify-center">
                           {chords.minor?.map((c, i) => (
                              <div key={c} className={`w-12 h-12 flex items-center justify-center rounded font-bold text-slate-300 border border-slate-600 bg-slate-800`}>
                                  {c}
                              </div>
                          ))}
                      </div>
          ...
          <div className="flex-1 flex flex-col gap-4">
              ...
              <div className="bg-slate-900 border border-slate-800 rounded p-3">
                  ...
                  <div className="flex gap-2 items-center">
                      <button onClick={() => setPlaying(!playing)} className="p-2 bg-fuchsia-900/50 text-fuchsia-400 border border-fuchsia-700 rounded hover:bg-fuchsia-800">
