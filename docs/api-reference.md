...
export type WidgetType = 
  'TRANSFORMER' | 'SCRATCHPAD' | 'FOCUS_HUD' | 'DEV_OPTIC' | 
  'CIPHER_VAULT' | 'CHROMA_LAB' | 'TEMPORAL' | 'SONIC' | 
  'CALC' | 'ASSET' | 'POLYGLOT' | 'WRITEPAD' | 'WEATHER' | 
  'VALUTA' | 'SYSTEM' | 'HELP' | 'ARCHITECT' | 'THEME_ENGINE' | 
  'GHOST' | 'RADIO' | 'SUDOKU' | 'DOCU_HUB' | 'GIT_PULSE' | 
  'PROJECT_TRACKER' | 'WEB_TERMINAL' | 'NEWS_FEED' | 
  'CIPHER_PAD' | 'PDF_VIEWER' | 'RESEARCH_BROWSER' | 
  'SECURE_CALENDAR' | 'MACRO_NET' | 'CHAIN_PULSE' | 
  'REG_RADAR' | 'MARKET' | 'STRATEGIC' | 'CLIPBOARD';
```
